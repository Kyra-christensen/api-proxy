...
export default function YelpSearch() {
  ...
  async function handleYelpSubmit(e) {
    ...
    const response = await fetch(`/.netlify/functions/yelp?search=${formLocation}`);
    // put the jsonified data in state and set the loading state to false
    const json = await response.json();
    setSearchResult(json.businesses);
    setIsLoading(false);
  }
  ...
  return (
    <section className='yelp'>
      ...
      <form onSubmit={handleYelpSubmit}>
        ...
        <label>
          City:
          <input placeholder="ex) Portland Or" onChange={e => setFormLocation(e.target.value)} />
        </label>
      ...
      {/* Make a BusinessesList component to import and use here. Use a ternery to display a loading spinner (make a <Spinner /> component for this) if the data is still loading. */}
      {
        isLoading
          ? <Spinner />
          : <BusinessesList searchResult={searchResult} />
      }
    </section>
  );
}
